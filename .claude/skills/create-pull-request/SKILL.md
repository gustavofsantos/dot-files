...
**Mermaid diagrams — use as a judgment call, not by default:**
...
(`A["label with (parens)"]`); long chains of `-->` are fine; `graph TD` and
`sequenceDiagram` are the most reliably rendered.
