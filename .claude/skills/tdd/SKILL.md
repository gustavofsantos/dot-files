...
Drives `type: implementation` issues only — an `investigation` issue is answered with Finn (dead-reckoning), not tested into existence.
...
When all tasks are checked, hand the branch to Victor (deep-review) before merge, plus `deslop`/`readable` for language cleanup. Archiving the issue is the human's call.
