...
Remove AI-generated cruft from the branch diff against `master`, touching **only changed lines** (`+`/`-`), without altering behavior, following the surrounding Clojure conventions.

## Workflow

`git diff master...HEAD` → scan only added/removed lines → fix clearest cases (one change per edit) → `lein cljstyle fix` + `clj-kondo` must pass → summarize in 1–3 sentences.

## What counts as slop here

- **Comments** that narrate the code, restate Clojure syntax, or reference Jira tickets. Keep comments that carry domain *why*.
