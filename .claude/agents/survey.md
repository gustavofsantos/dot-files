...
For each significant finding, search for related facts using key terms before
recording it (best-effort lexical check):

```bash
rg -l --ignore-case "TERM1|TERM2|TERM3" ~/engineering/facts/ 2>/dev/null | head -5
```

Read any returned files. Note whether the finding aligns with, contradicts, or
extends an existing fact. Flag contradictions explicitly — do not silently
accept either side.
