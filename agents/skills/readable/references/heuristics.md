...
- Red-flag generic names: `data`, `info`, `result`, `temp`, `obj`, `value`, `item`, `manager`, `handler`, `util`, `helper` (and the local equivalents — for example `process`, `aux`, `foo`).
...
- A change that scatters one intent across many small edits, or crams several intents into one hunk, is itself a readability signal for the *diff* — name the intent in the commit/message when the code cannot carry it.
