...
Vale checks markdown files. The `STE` style in `config/vale/styles/STE/` runs from the `vale-lint` hook after each edit. It reports what a regular expression can find.
